...
function createCountryList(){
    const num_countries = country_list.length;   //number of countries 

    let i = 0 ,
    ul_list_id;
    country_list.forEach((country,index)=>{
        if(index % Math.ceil(num_countries/num_of_ul_lists)==0){    //no of countries devided in each list
              ul_list_id=`list-${i}`;
...
num_of_ul_lists=3;   //total 3 lists
createCountryList();
...
function resetCountryList(){ // this function is for to reset the all the lists 
    country_list.forEach(country =>{
        document.getElementById(country.name).classList.remove("hide");
    })
}